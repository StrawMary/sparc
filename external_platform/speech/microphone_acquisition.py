import sys
sys.path.insert(0, '/home/amiro/installers/naoqi/pynaoqi-python2.7-2.5.5.5-linux64/lib/python2.7/site-packages')
...
class PepperSpeechRecognitionEngine:
	def __init__(self, play_back=True):
		...
		self.port = 9559
		self.audio_proxy = ALProxy('ALAudioDevice', self.ip, self.port)
		...
		self.silence = chr(0)*self.CHUNK*self.CHANNELS_NO*2
	...
	def on_audio_received(self, data):
		...
		if self.play_back:
			if output == "":
				output = self.silence
			self.output_stream.write(output)
			self.play_on_pepper(output)

	def play_on_pepper(self, audio_data):
		pass
		#AICI TREBUIE SA VEZI CUM FACI CA AUDIO DATA SA FIE CANTAT PE PEPPER
		self.audio_proxy.sendLocalBufferToOutput(len(audio_data), 0)
...
ps = PepperSpeechRecognitionEngine(play_back=True)
ps.listen()
